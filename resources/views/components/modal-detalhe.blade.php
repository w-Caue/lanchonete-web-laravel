<div x-cloak x-data="{ isModalOpen: false }" x-on:open-detalhe.window="isModalOpen = true"
    x-on:close-detalhe.window="isModalOpen = false">
    <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150"
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
        class="fixed inset-0 z-50 flex items-end bg-black bg-opacity-35 sm:items-center sm:justify-center">
        <!-- Modal -->
