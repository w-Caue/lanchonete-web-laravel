        ...
        @if ($encarte)
            <div wire:init="load" class="flex flex-wrap gap-3 my-1 mx-5">
                @foreach ($encarte->produtos as $produto)
                    <div x-on:mouseover="toolproduct = 'product'" x-on:mouseleave="toolproduct = 'nenhum'"
                        x-data="{ toolproduct: 'nenhum' }" wire:key="{{ $produto->id }}"
                        ...
                        <div class="w-full flex flex-col gap-3 p-5">
                            <div wire:click="produto({{ $produto->id }})" class="py-2 text-gray-600">
                                <h5 class="my-3 text-md uppercase font-semibold tracking-wider">{{ $produto->nome }}
                                </h5>
                                <p class="mb-4 text-xs uppercase font-semibold opacity-65">{{ $produto->descricao }}</p>
                        ...
                        <div x-show="toolproduct === 'product'" x-transition:enter="transition ease-out duration-300"
                            x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100"
                            x-transition:leave="transition ease-in duration-300"
                            x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90"
                            class="absolute bottom-0 flex justify-center ml-7 mb-2 gap-2 ">

                            <a class="text-sm text-white bg-purple-500 font-semibold tracking-widest rounded py-2 px-2">
                                Ver Produto
                            </a>
                            ...
                            <div class="flex justify-center border rounded bg-white">
                                ...
                                @foreach ($carrinho as $index => $item)
                                    @if ($produto->id == $item['codigo'])
                                        <input class="w-12 mx-2 text-center" type="text"
                                            value="{{ $carrinho[$index]['quantidade'] }}">
                                    @else
                                        {{-- <input class="w-12 mx-2 text-center" type="text"
                                            value="0"> --}}
